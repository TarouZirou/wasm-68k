...
| 6 | Stable core/Wasm APIs, X68S v8 state manifest/CRC/LZ4, immutable-media exclusion, IndexedDB, D&D, CRT shader, diagnostics, issue form, Pages workflow, base-path Chromium tests, GPL exception and `cargo deny` gate. | ROMs and game media are never automatically saved, sent, or committed. |
