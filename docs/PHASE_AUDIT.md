...
| 6 | Stable core/Wasm APIs, X68S v9 state manifest/CRC/LZ4, immutable-media exclusion, IndexedDB, D&D, CRT shader, diagnostics, issue form, Pages workflow, base-path Chromium tests, GPL exception and `cargo deny` gate. | User-selected ROMs are retained only in browser-local IndexedDB with an explicit clear action. Game media is never automatically saved, sent, or committed. |
